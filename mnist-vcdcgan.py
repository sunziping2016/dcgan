...
class Generator(nn.Module):
    ...
    def forward(self, z, label):
        ...
        x = F.relu(self.bn3(self.cvt1(torch.cat([x.view(-1, 128, 7, 7), label.expand(x.size(0), 10, 7, 7)], 1))))
        return F.sigmoid(self.cvt2(torch.cat([x, label.expand(x.size(0), 10, 14, 14)], 1)))

class Discriminator(nn.Module):
    def __init__(self):
        super(Discriminator, self).__init__()
        self.cv1 = nn.Conv2d(1 + 10, 64, 4, 2, 1)
        self.cv2 = nn.Conv2d(64 + 10, 128, 4, 2, 1)
        self.bm1 = nn.BatchNorm2d(128)
        self.fc1 = nn.Linear(128 * 7 * 7 + 10, 1024)
        self.bm2 = nn.BatchNorm1d(1024)
        self.fc2 = nn.Linear(1024 + 10, 1)
    def forward(self, x, label):
        label_expand = label.view(-1, 10, 1, 1)
        x = F.leaky_relu(self.cv1(torch.cat([x, label_expand.expand(x.size(0), 10, 28, 28)], 1)))
        x = F.leaky_relu(self.bm1(self.cv2(torch.cat([x, label_expand.expand(x.size(0), 10, 14, 14)], 1))))
        x = F.leaky_relu(self.bm2(self.fc1(torch.cat([x.view(-1, 128 * 7 * 7), label], 1))))
...
images = torch.stack([mnist[i][0] for i in range(len(mnist))])
orig_labels = torch.LongTensor([mnist[i][1] for i in range(len(mnist))])
...
def plt_gen_with_random_noise(index):
    ...
    if cuda:
        z = z.cuda()
    fig = plt_images(G(z, plt_labels).data.cpu().squeeze(1).numpy(), 10, 10)
...
def plt_gen_with_fixed_noise(index):
    fig = plt_images(G(plt_z, plt_labels).data.cpu().squeeze(1).numpy(), 10, 10)
...
def plt_tsne(index):
    ...
    if cuda:
        z = z.cuda()
    source, target = images[indices].cpu().view(n_tsne_sample, 28 * 28).numpy(), G(z, label).data.cpu().view(n_tsne_sample, 28 * 28).numpy()
